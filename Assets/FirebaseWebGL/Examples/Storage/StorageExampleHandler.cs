...
namespace FirebaseWebGL.Examples.Storage
{
    public class StorageExampleHandler : MonoBehaviour
    {
        ...
        {
            FirebaseStorage.UploadFile(filePathInputField.text,
                Convert.ToBase64String(Encoding.ASCII.GetBytes(fileDataInputField.text)), gameObject.name,
                "DisplayInfo", "DisplayErrorObject");
        }

        public void DownloadFile()
        {
            FirebaseStorage.DownloadFile(filePathInputField.text, gameObject.name, "DisplayFile", "DisplayErrorObject");
        }
